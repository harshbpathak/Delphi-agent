...
let pollOffset = 0;
let statusProvider: (() => Promise<string>) | null = null;
let holdingsProvider: (() => Promise<string>) | null = null;
...
async function pollUpdates() {
    ...
    try {
        ...
        for (const upd of data.result || []) {
            ...
            if (String(msg.chat.id) !== chatId) continue; // only the owner controls the agent
...
export async function startTelegram(
    providers: { status: () => Promise<string>; holdings: () => Promise<string> }
) {
    if (!token) {
    ...
    if (!chatId) chatId = await getState('telegramChatId');
    pollUpdates();
