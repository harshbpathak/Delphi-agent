...
export async function selfCalibrate() {
    try {
        ...
        for (const c of cats) {
            ...
            if (sub.length >= MIN_CATEGORY_N)
                byCategory[c] = fitWeight(sub).w;
        }
        const learned = {
            global: globalFit.w,
            byCategory,
        ...
        console.warn('  [Calibrate] failed:', e.message?.slice(0, 150));
    }
}
function fitWeightAt(samples, w) {
    let loss = 0;
    for (const s of samples) {
        const p = clamp(sigmoid((1 - w) * logit(s.raw) + w * logit(s.market)), EPS, 1 - EPS);
        loss += -(s.y * Math.log(p) + (1 - s.y) * Math.log(1 - p));
