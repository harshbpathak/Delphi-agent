...
export type Ambiguity = 'none' | 'minor' | 'severe';

export interface ConsensusInput {
    /** Our raw estimate for outcome 0 */
    rawProb: number;
    /** Market implied probability for outcome 0 */
    marketProb: number;
    /** Hours until the question resolves (null if unknown) */
    hoursToResolve: number | null;
    trades24h: number;
    uniqueWallets24h: number;
    /** Model believes the underlying factual matter is already settled */
    eventConcluded: boolean;
    /** Model's read on whether the resolution criteria admit conflicting readings */
    ambiguity: Ambiguity;
...
export function applyMarketConsensus(input: ConsensusInput): ConsensusResult {
    ...
    if (disagreement > 0.50) w += 0.30;
    else if (disagreement > 0.30) w += 0.20;
    else if (disagreement > 0.15) w += 0.10;
    ...
    if (hoursToResolve !== null) {
        let timeAdd = 0;
        if (hoursToResolve < 6) timeAdd = 0.25;
        else if (hoursToResolve < 24) timeAdd = 0.15;
        else if (hoursToResolve < 72) timeAdd = 0.05;
        w += timeAdd * Math.min(1, disagreement / 0.25);
    }

    // 3. Depth of aggregation
    if (uniqueWallets24h >= 10) w += 0.15;
    else if (uniqueWallets24h >= 5) w += 0.08;
    if (trades24h < 5) w -= 0.10; // a near-empty book is weak evidence

    // 4. Ambiguous rules: our reading is the risk, not the facts
    if (ambiguity === 'severe') w += 0.30;
    else if (ambiguity === 'minor') w += 0.12;

    // 5. The one legitimate override
    if (verifiedFact) w -= 0.45;
