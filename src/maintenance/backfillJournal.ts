...
async function main() {
    ...
    const onchain = await db.all(
        `SELECT market, outcome_idx AS outcomeIdx,
         ...
         GROUP BY market, outcome_idx`,
        [us]
    );

    let inserted = 0;
    for (const row of onchain as any[]) {
        const j = await db.get(
            `SELECT COALESCE(SUM(shares),0) AS shares, COALESCE(SUM(cost_tokens),0) AS cost
             FROM trades WHERE market_address = ? AND outcome_idx = ? AND dry_run = 0`,
            [row.market, row.outcomeIdx]
        );

        const missingShares = row.shares - (j?.shares || 0);
        if (missingShares <= EPS) continue;
        ...
        await db.run(
            `INSERT INTO trades (market_address, question, category, outcome_idx, outcome_label,
             ...
             VALUES (?, ?, ?, ?, ?, NULL, ?, ?, ?, ?, 0, 'backfill', ?)`,
            [row.market, question, category, row.outcomeIdx, label,
    ...
    const check = await db.get(
        `SELECT COUNT(*) n, COALESCE(SUM(pnl_tokens),0) pnl,
