...
import { getCombinedProbability, CombinedPrediction } from './intelligence/index.js';
...
let lastMarkets: EnrichedMarket[] = [];
let lastPosture: CompetitionPosture | null = null;
let lastBankroll = 0;
let activeGuardrails: RiskGuardrails = DEFAULT_GUARDRAILS;
...
function sharesToBigint(shares: number): bigint {
    return BigInt(Math.round(shares * 1e6)) * 1_000_000_000_000n; // µshare precision → 18 decimals
}
function tokensFromBigint(v: bigint): number {
    return Number(v) / 10 ** TOKEN_DECIMALS;
}
function sharesFromBigint(v: bigint): number {
    return Number(v / 1_000_000_000_000n) / 1e6;
}
            ...
            try {
                ...
                if (status === 'settled') {
                    ...
                    if (winningIdx !== null && Number(pos.outcomeIdx) !== winningIdx) {
                        ...
                        if (lostCost <= 0.01) continue; // already journalled (or nothing at risk)
    ...
    for (const pos of positions) {
        try {
            ...
            if (!freeRolled && gainPct !== null && gainPct >= FREE_ROLL_GAIN_PCT
                ...
                if (sellShares > sharesIn) sellShares = sharesIn;
                ...
                if (!DRY_RUN) {
                    const res = await delphiClient.sellShares({
                        ...
                        outcomeIdx: idx,
                        sharesIn: sellShares,
                        minTokensOut: quote.tokensOut * (10_000n - SELL_SLIPPAGE_BPS) / 10_000n,
                    });
...
async function circuitBreakerTripped(bankroll: number): Promise<boolean> {
...
interface TradeCandidate {
    ...
    outcomeIdx: number;
    /** P(chosen outcome wins) */
    effectiveProb: number;
    /** Spot price of chosen outcome */
    spotPrice: number;
    ...
    crowdBoost: boolean;
    /** Crowd has been buying OUR side hard (momentum confirmation) */
...
function scoreCandidate(c: TradeCandidate): number {
    ...
    if (c.momentumBoost) s *= 1.05; // sentiment blowing our way
...
function computeMomentumBoost(outcomeIdx: number, buyVol0: number, buyVol1: number): boolean {
    const ours = outcomeIdx === 0 ? buyVol0 : buyVol1;
    const theirs = outcomeIdx === 0 ? buyVol1 : buyVol0;
...
async function shouldEvaluate(market: EnrichedMarket, articles: ScrapedArticle[]): Promise<string | null> {
    const lastEval = await getLastEvaluation(market.address);
    if (!lastEval) return 'never evaluated';
    if (articles.length > 0) return `${articles.length} new articles`;
...
async function evaluateMarkets(markets: EnrichedMarket[], guardrails: RiskGuardrails): Promise<TradeCandidate[]> {
    const candidates: TradeCandidate[] = [];
    let evals = 0;
    let consecutiveGeminiFailures = 0;

    for (const market of markets) {
        if (evals >= MAX_EVALS_PER_LOOP) break;
...
async function executeCandidate(c: TradeCandidate, bankroll: number, guardrails: RiskGuardrails): Promise<number> {
    ...
    const provisionalTokens = calculatePositionSize(
        c.effectiveProb, c.spotPrice, market.tradingFee, bankroll, guardrails
    );
    ...
    let tokensIn = await getSlippageQuote(market.address as `0x${string}`, outcomeIdx, sharesOut);
    if (!tokensIn) {
    ...
    if (finalTokens <= 0) return 0;

    const quotedCost = tokensFromBigint(tokensIn);
    if (quotedCost > finalTokens * 1.02) {
        const scale = finalTokens / quotedCost;
        sharesOut = sharesOut * BigInt(Math.round(scale * 1e6)) / 1_000_000n;
        tokensIn = await getSlippageQuote(market.address as `0x${string}`, outcomeIdx, sharesOut);
        if (!tokensIn) {
    ...
    let maxTokensIn = tokensIn * (10_000n + MAX_SLIPPAGE_BPS) / 10_000n;
    ...
    if (maxCostHuman > capSpend && maxCostHuman <= capSpend * 1.15) {
        ...
        sharesOut = sharesOut * BigInt(Math.round(scale * 1e6)) / 1_000_000n;
        const requote = await getSlippageQuote(market.address as `0x${string}`, outcomeIdx, sharesOut);
        if (!requote) return 0;
        tokensIn = requote;
        effectivePrice = tokensFromBigint(tokensIn) / sharesFromBigint(sharesOut);
        maxTokensIn = tokensIn * (10_000n + MAX_SLIPPAGE_BPS) / 10_000n;
    ...
    if (DRY_RUN) {
        ...
        txHash = await executeTrade(market.address as `0x${string}`, outcomeIdx, sharesOut, maxTokensIn);
        if (!txHash) return 0;
    }
    ...
    await notify(
        `${DRY_RUN ? '🏜️ DRY RUN' : '🚀 TRADE'}: bought *${sharesFromBigint(sharesOut).toFixed(2)}* shares of *"${label}"*\n` +
...
async function runLoop() {
    ...
    for (const c of candidates) {
        if (loopTradesExecuted >= MAX_TRADES_PER_LOOP) break;
...
async function getHoldings(): Promise<Array<{ market: string; question: string; outcome: string; shares: number; mark: number; status: string }>> {
    try {
        const signer = await delphiClient.getSigner();
        const { positions } = await delphiClient.listPositions({
            wallet: signer.address,
        ...
        for (const pos of positions) {
            ...
            let status: string = m ? 'open' : pos.marketStatus;
...
async function holdingsText(): Promise<string> {
    const h = await getHoldings();
    if (h.length === 0) return 'No open positions.';
