...
export const heldMarketsNeedingRecheck = new Set();
// ─── math helpers ────────────────────────────────────────────────────────────
function normCdf(z) {
...
function detectCrypto(m) {
    const q = m.question;
    if (!/coingecko|daily close/i.test(q))
        return null;
...
async function runCrypto(m, spec) {
    if (Date.now() > spec.measureTs)
        return;
    const res = await fetch(`https://api.coingecko.com/api/v3/simple/price?ids=${spec.coinId}&vs_currencies=usd`, { signal: AbortSignal.timeout(15_000) });
    if (!res.ok)
...
export async function runDataWatchers(markets) {
    ...
    for (const addr of new Set(held)) {
        ...
        if (!market)
            continue; // closed for trading — brackets can't act anyway
        try {
            ...
            if (usgs)
                await runUsgs(market, usgs);
            else if (crypto)
                await runCrypto(market, crypto);
        }
        catch (e) {
            console.warn(`  [Watch] ${addr.slice(0, 10)} failed:`, e.message?.slice(0, 120));
        }
    }
}
